...
const store = {
  async get(key, shared = true) {
    try {
      if (!hasStore)
        return mem.has(key + shared) ? mem.get(key + shared) : null;
    ...
    try {
      if (!hasStore)
        return [...mem.keys()]
          ...
          .map((k) => k.slice(0, k.length - String(shared).length));
...
export default function App() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    const list = (await Promise.all(keys.map((k) => store.get(k)))).filter(
      Boolean,
    );
  ...
  if (booting)
    return (
      <Shell>
        <p className="py-16 text-center text-sm text-slate-600">載入中…</p>
      </Shell>
    );
  if (!user)
    return (
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="mt-1 text-xs text-slate-500">
                點選大家有空的時段,自動算出最多人能出征的時間
              </p>
            </div>
          </button>
          {user && (
            <div className="flex items-center gap-2 text-sm">
              <span className="hidden rounded-md bg-slate-800 px-2.5 py-1 font-semibold text-amber-400 sm:inline">
                {user.name}
              </span>
              <button
                onClick={onLogout}
                className="flex items-center gap-1 rounded-md px-2 py-1.5 text-slate-400 hover:bg-slate-800 hover:text-slate-200"
              >
    ...
    <div className="mx-auto max-w-sm pt-6">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="mb-1 flex items-center gap-2 text-amber-400">
          ...
          <h2 className="text-sm font-bold">登入 / 註冊</h2>
        </div>
        <p className="mb-5 text-xs text-slate-500">
          設定暱稱與密碼,之後這台裝置免再輸入。新暱稱會自動建立。
        </p>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center overflow-hidden rounded-lg border border-slate-800 bg-slate-900">
            ...
            <button
              onClick={joinByCode}
              className="border-l border-slate-800 px-3 py-2 text-sm text-amber-400 hover:bg-slate-800"
            >
              加入
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {events.map((ev) => (
            <div
              key={ev.id}
              className="group relative rounded-xl border border-slate-800 bg-slate-900/60 p-4 transition hover:border-amber-700/60 hover:bg-slate-900"
            >
              <button
  ...
  return (
    <div>
            ...
            <div className="flex gap-2">
              {[30, 60].map((s) => (
                <button
                  key={s}
                  onClick={() => setStep(s)}
                  className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${step === s ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                >
                  {s} 分鐘
                </button>
              ))}
              ...
              {days.map((d, i) => {
                ...
                return (
                  <button
                    key={k}
                    onClick={() => toggleDay(d)}
                    className={`aspect-square rounded-md text-sm transition ${sel ? 'bg-amber-500 font-bold text-slate-950' : 'text-slate-300 hover:bg-slate-800'}`}
                  >
                    {d.getDate()}
                  </button>
                );
      ...
      <button
        ...
        className={`mt-6 w-full rounded-lg py-3 text-sm font-bold transition ${valid ? 'bg-amber-500 text-slate-950 hover:bg-amber-400' : 'bg-slate-800 text-slate-600'}`}
      >
        建立遠征
      </button>
    </div>
...
function EventView({ eventId, user, onBack, onParticipantsChange }) {
  ...
  if (loading)
    return <p className="py-12 text-center text-sm text-slate-600">載入中…</p>;
  if (!ev)
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-slate-500">找不到這場遠征。</p>
        <button onClick={onBack} className="mt-3 text-sm text-amber-400">
          ← 返回列表
        </button>
      </div>
    );
  ...
  return (
    <div>
          ...
          <div className="flex rounded-lg bg-slate-800 p-0.5 text-sm">
            {[
              ...
              <button
                key={k}
                onClick={() => setTab(k)}
                className={`rounded-md px-3 py-1.5 font-medium transition ${tab === k ? 'bg-amber-500 text-slate-950' : 'text-slate-300 hover:text-white'}`}
              >
                {label}
              </button>
            ))}
      ...
      <div className="mb-4 flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/50 px-3 py-2 text-xs text-slate-400">
        ...
        遠征代碼{' '}
        <span className="rounded bg-slate-800 px-1.5 py-0.5 font-mono font-semibold text-amber-400">
          {eventId}
        </span>
      ...
      {tab === 'mine' && (
        <p className="mb-3 text-xs text-slate-500">
          以 <span className="font-semibold text-amber-400">{user.name}</span>{' '}
          填寫 · 拖曳塗選有空時段
        </p>
      )}
      ...
      {tab === 'mine' && (
        ...
        >
          {saved ? (
            ...
            <>
              <Save className="h-4 w-4" strokeWidth={1.5} /> 儲存我的時間
            </>
          )}
        </button>
      )}
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="rounded-xl border border-amber-800/40 bg-gradient-to-br from-amber-950/40 to-orange-950/20 p-4">
            <h3 className="flex items-center gap-2 text-sm font-bold text-amber-300">
              <Crown className="h-4 w-4" /> 最多人可以的時段
            </h3>
            {total === 0 ? (
              <p className="mt-2 text-sm text-slate-400">還沒有人填寫時間。</p>
            ) : maxCount === 0 ? (
                ...
                <div className="mt-3 space-y-1.5">
                  {groupBest(bestCells, ev.step).map((g, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg bg-slate-900/60 px-3 py-2 text-sm"
                      >
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
            <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-300">
              <Users className="h-4 w-4" /> 已回覆 ({total})
            </h3>
            {total === 0 ? (
              <p className="text-sm text-slate-500">尚無人回覆。</p>
            ) : (
            ...
            <p className="mt-3 flex items-center gap-2 text-[11px] text-slate-600">
              顏色越紅代表越多人有空
              <span
                className="inline-flex h-3 w-24 rounded-sm"
                style={{
                  background: `linear-gradient(90deg, ${heatColor(0.01)}, ${heatColor(0.5)}, ${heatColor(1)})`,
                }}
              />
            </p>
          </div>
...
function CellDetail({ cellKey: ck, people, avails }) {
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-3 text-sm">
      ...
      {no.length > 0 && (
        <span className="ml-2 text-slate-500">缺:{no.join('、')}</span>
      )}
    </div>
  );
}
